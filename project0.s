...
li $v0, 4 # print string
la $a0, 3($s0) # so that the part of string s0 from index 3 gets printed
syscall
lb $t0, 3($s0) # temporarily store the value of index 3 in $t0
sb $zero, 3($s0) # temporarily set index 3 to null char
la $a0, 0($s0) # so that the string gets printed from beginning to null char
syscall
sb $t0, 3($s0) # revert the character in index 3 to previous value
...
syscall
lb $t0, 5($s0) # temporarily store the value of index 5 in $t0
sb $zero, 5($s0) # temporarily set index 5 to null char
la $a0, 0($s0) # so that the string gets printed from beginning to null char
syscall
sb $t0, 5($s0) # revert the character in index 5 to previous value
